...
Use a one-second neighborhood half-width. For L windows, set b=min(L-1, max(1, floor(fps/W))) and V[t,s]=L/b for 0<|t-s|<=b, otherwise zero. When L=1, set V=0. In exact arithmetic this is the reference ASOT kernel with radius b/L, expressed in fixed approximate physical time. Production constructs the integer-width kernel directly from b and amplitude L/b; never recover b by truncating L*(b/L). Upstream parity cases must first assert int(L*(b/L))==b (safe examples: b=4 with L=16,32,64). Test problematic lengths such as L=49 separately against the explicit production kernel; do not change b or silently nudge the reference radius. Save the achieved radius in seconds. Do not use the optional monotonic action-order prior. A neighborhood consistency penalty favors nearby agreement; it does not learn a directional action grammar.
...
- inverse-permutation round trips, original-span scoring, A0 equivariance, and preserved counts
...
Artifact identity includes protocol version 1.3, the unhalved cosine convention, direct integer kernel construction, and each diagnostic's restoration mode. Old half-scale cosine results or un-restored permutation metrics are not current-protocol results.
...
- a main budgeted-algorithm table with capped counts/residuals, and secondary convergence-qualified paired comparisons with explicit sparse/empty coverage
...
Stage A's detailed agent instructions are supplied in STAGE_A_AGENT_INSTRUCTIONS.md (version 1.3). Stage B and C numerical defaults should be reviewed for feasibility at their implementation handoff using runtime and label-free diagnostics; any change becomes a new frozen configuration before action evaluation, not silent tuning of this document.
